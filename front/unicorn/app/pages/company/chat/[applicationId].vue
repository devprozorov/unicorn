<template>
  <div class="page">
    ...
    <section class="wrap">
      <div class="hero">
        <div class="hero-left">
          <h1 class="hero-title">{{ t('companyChat.title') }}</h1>
          <p class="hero-sub">{{ t('companyChat.description') }}</p>
        </div>

        <div class="hero-actions">
          <NuxtLink class="hero-btn" to="/company/chat">{{ t('companyChat.backToChats') }}</NuxtLink>
        </div>
      </div>
      ...
      <div class="chat-shell">
        ...
        <div ref="box" class="chat-box">
          ...
          <div v-else class="msg-list">
            <div
              ...
              :key="m.messageId"
              class="msg-row"
              :class="m.senderType === 'company' ? 'me' : 'them'"
            >
              <div class="bubble">
                <div class="meta">
                  <span class="who">{{ m.senderType === 'company' ? t('companyChat.company') : t('companyChat.candidate') }}</span>
                  <span class="sep">??</span>
        ...
        <div class="composer">
          <textarea
            v-model="text"
            class="inp"
            :placeholder="t('companyChat.writePlaceholder')"
            rows="2"
            :disabled="sending"
            @keydown="onKeydown"
          />
          <button class="send" @click="send" :disabled="sending || !text.trim()">
            {{ sending ? t('companyChat.sending') : t('companyChat.send') }}
          </button>
        </div>
      </div>
    </section>
...
function chatLastSeenKey(cid: string, appId: string) {
  return `unicorn:company:chat:last_seen:${cid || 'anon'}:${appId}`
}

function saveChatLastSeen(cid: string, appId: string, ts: number) {
  if (!process.client) return
  try {
    localStorage.setItem(chatLastSeenKey(cid, appId), String(ts || 0))
  } catch {}
}

function markChatSeen() {
  if (!process.client) return
  const cid = String((auth as any)?.user?.userId || (auth as any)?.profile?.id || (auth as any)?.me?.id || 'anon')
  const last = messages.value[messages.value.length - 1]
  if (!last?.createdAt) return
  const ts = Date.parse(String(last.createdAt))
  if (Number.isNaN(ts)) return
  saveChatLastSeen(cid, applicationId.value, ts)
}
...
async function load(forceScroll = false) {
  ...
  try {
    ...
    if (code === 'mfa_required') {
      hint.value = t('companyChat.enable2fa')
    } else {
      err.value = getErrorMessage(code || 'load_failed', t, 'companyChat.errors.loadFailed')
    }
  } finally {
    loading.value = false
  }
...
async function send() {
  ...
  try {
    await axios.post(`/chat/${applicationId.value}/messages`, { text: v })
    text.value = ''
    await load(true)
  } catch (e: any) {
    const code = e?.response?.data?.error
    err.value = getErrorMessage(code || 'send_failed', t, 'companyChat.errors.sendFailed')
  } finally {
    sending.value = false
  }
}
